...
if form.getvalue('submit'):
	...
	log = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')+' surname: '+form.getvalue('surname')+' result: '+str(k)+'/'+str(c)+'/'+str(n-1)+' - '+str(100*k/float(n-1))+'%\n'
	...
	while getexistence(n):
		...
		n+=1
	print "\t\t\t<p>"
	print '\t\t\t\tsurname: <input type="text" name="surname" value=""/> <br/>'
